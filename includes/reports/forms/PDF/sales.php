...
if($_SESSION["language"] == "ar_EG"){
	$this->data .= "<div id='bigi'>تقارير المبيعات</div>";
}else {
	$this->data .= "<div id='bigi'>Sales Reports</div>";
}
...
// ITEMS
if($_SESSION["language"] == "ar_EG"){
	$this->data .= "<table id='items'><tr><th>رقم المرجع</th><th>العميل</th><th>المنتج</th><th>الاجمالي</th><th>التاريخ</th></tr>";
} else {
	$this->data .= "<table id='items'><tr><th>Reference No</th><th>Customer</th><th>Product Name</th><th>Total</th><th>Date</th></tr>";
...
if (count($this->totals)>0) { foreach ($this->totals as $t) {
	$this->data .= "<tr class='ttl'><td class='left' colspan='4'>$t[0]</td><td>$t[1]</td></tr>";
}}
$this->data .= "</table>";
