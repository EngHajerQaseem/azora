...
if(isset($_POST['page'])){
    ...
    $whereSQL = "WHERE services.archive = 0 ";
     $orderSQL = '';
    $whereSQL2 = "where services.archive = 0 " ;
    $orderSQL ='';
    ...
    if(!empty($keywords)){
        ...
        if($_SESSION["language"] == "ar_EG"){
            ...
            (services.name_ar LIKE '%".$keywords."%' AND services.name IS NULL)
            OR
            (services.name LIKE '%".$keywords."%' AND services.name_ar IS NULL)
            OR
            (services.name_ar LIKE '%".$keywords."%')) ";
    ...
    //get number of rows
    $queryNum = $mysqli->query("SELECT COUNT(*) as userNum FROM services ".$whereSQL.$orderSQL);
    ...
    //get rows
    $query = $mysqli->query("SELECT services.id as id, services.name as ProductName,services.name_ar as ProductNameAr, services.price as product_price from services  $whereSQL2 Limit $start,$limit");
        ...
        <table class="table table-hover ">
            <thead>
                <tr>
                    ...
                    <th style="width:15em"><?php echo _("product_Name");?></th>
                    <!-- <th style="width:8em"><?php echo _("product_stock");?></th> -->
                    <th style="width:8em"><?php echo _("product_price");?></th>
            ...
            <tbody>
                <tr style="cursor: pointer;"
                    onclick="window.location='service_details.php?id=<?php echo $row["id"];?>'">
