...
class Pagination{
	...
	var $prevLink		= '<';
	var $lastLink		= '&raquo;';
	var $fullTagOpen	= '<nav  id="pagination-container"> 
	...
	function getAJAXlink( $count, $text) {
        if($this->link_func == '' && $this->contentDiv == '')
            return '<li class="page-item"> <a class="page-link" href="'.$this->baseURL.'?'.$count.'"'.$this->anchorClass.'>'.$text.'</a>';
		
		$pageCount = $count?$count:0;
		if(!empty($this->link_func)){
			$linkClick = 'onclick="'.$this->link_func.'('.$pageCount.',1)"';
		}else{
			$this->additionalParam = "{'page' : $pageCount}";
			$linkClick = "onclick=\"$.user('". $this->baseURL."', ". $this->additionalParam .", function(data){
